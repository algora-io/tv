defmodule AlgoraWeb.ProfileLive.VideoEntryComponent do
    ...
    <div class="sm:grid sm:grid-cols-2 sm:gap-2 sm:items-start sm:border-t sm:border-gray-200 sm:pt-2">
      <div class="border border-gray-300 rounded-md px-3 py-2 mt-2 shadow-sm focus-within:ring-1 focus-within:ring-indigo-600 focus-within:border-indigo-600">
        <label for="name" class="block text-xs font-medium text-gray-900">
          <%= if @duration do %>
            Title <span class="text-gray-400">(<%= MP4Stat.to_mmss(@duration) %>)</span>
          <% else %>
            Title
            <span class="text-gray-400">
              (calculating duration
              <Heroicons.arrow_path class="inline-block animate-spin h-2.5 w-2.5 text-gray-400" />)
            </span>
          <% end %>
        </label>
        <input
          type="text"
          name={"videos[#{@ref}][title]"}
          value={@title}
      ...
      <div class="border border-gray-300 rounded-md px-3 py-2 mt-2 shadow-sm focus-within:ring-1 focus-within:ring-indigo-600 focus-within:border-indigo-600">
        ...
        <input
          type="text"
          name={"videos[#{@ref}][artist]"}
          value={@artist}
          class="block w-full border-0 p-0 text-gray-900 placeholder-gray-500 focus:ring-0 sm:text-sm"
        />
      </div>
      <div class="col-span-full sm:grid sm:grid-cols-2 sm:gap-2 sm:items-start">
        <.error input_name={"videos[#{@ref}][title]"} field={:title} errors={@errors} class="-mt-1" />
        <.error input_name={"videos[#{@ref}][artist]"} field={:artist} errors={@errors} class="-mt-1" />
      </div>
