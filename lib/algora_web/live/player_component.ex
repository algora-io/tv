defmodule AlgoraWeb.PlayerComponent do
  ...
  def render(assigns) do
    ...
    <div class="relative">
      <video
        ...
        data-muted-overlay="true"
        controls
        data-media-player
      />
      <div 
        class={"muted-overlay absolute top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50 cursor-pointer #{if !@show_muted_overlay, do: "hidden"}"}
        phx-click="unmute_video"
      >
  ...
  @impl true
  def handle_event("unmute_video", _, socket) do
    {:noreply, 
     socket
     |> push_event("unmute_video", %{player_id: socket.assigns.id})
     |> assign(:show_muted_overlay, false)}
  end
